...
import { RiUserAddLine, RiMailSendLine, RiFingerprint2Line, RiEyeLine, RiEyeOffLine } from "react-icons/ri";
...
const Signup = () => {
  ...
  const handleFormSubmit = async (event) => {
    ...
    setLoading(true);
    try {
      // Integration with your AuthService
      ...
      // Error handling assumes your service throws an error with a message
      toast.error(error.response?.data?.message || "Registration failed. System rejected credentials.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="w-full max-w-md bg-slate-900/40 border border-slate-800/60 p-8 backdrop-blur-xl rounded-2xl shadow-2xl">
        ...
        <div className="mt-10">
          ...
          <div className="mt-8 grid grid-cols-2 gap-4">
            <Button variant="outline" type="button" className="border-slate-800 bg-slate-950 hover:bg-slate-900 text-slate-300 h-12 transition-all">
