...
export default function PlansPage() {
  ...
  const handleUpgrade = async (planName) => {
    const loadingToast = toast.loading(`Initializing ${planName} request...`);
    try {
      // payload matches PaymentController.java expectations
      const response = await axios.post(`${import.meta.env.VITE_API_BASE_URL}/api/v1/payments/create-checkout-session`, {
        planName: planName, 
        userId: user.id
      });
      ...
      toast.error("Security handshake failed. Check backend connectivity.", { id: loadingToast });
      console.error(error);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white p-6 md:p-12  selection:bg-red-600/30">
      <div className="max-w-5xl mx-auto">
        <Button variant="ghost" onClick={() => navigate(-1)} className="mb-8 text-slate-400 uppercase font-black italic text-xs tracking-widest p-0 hover:bg-transparent hover:text-white transition-colors">
        ...
        <p className="text-slate-500 font-mono text-sm mb-12 uppercase tracking-widest">Select a clearance level to bypass restricted data firewalls.</p>
            ...
            <Card key={plan.id} className="bg-slate-900 border-slate-800 rounded-none relative overflow-hidden group">
              ...
              <CardHeader className="border-b border-slate-800 p-8 flex flex-row justify-between items-center">
                <div>
                  ...
                  <span className="text-[10px] font-bold text-red-500 uppercase tracking-widest">Term: {plan.duration}</span>
                </div>
              ...
              <CardFooter className="p-8 pt-0">
                ...
                >
                  {user?.planType === plan.id ? "CURRENT ACCESS" : <>Purchase Token <Zap className="ml-2 h-4 w-4 fill-current" /></>}
                </Button>
              </CardFooter>
            </Card>
          ))}
